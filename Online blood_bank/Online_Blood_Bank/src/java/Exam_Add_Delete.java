...
@WebServlet(urlPatterns = {"/Exam_Add_Delete"})
public class Exam_Add_Delete extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
              String blood_id = request.getParameter("blood_id");
               try {
                ...
                int i = st.executeUpdate("DELETE FROM blood_detail WHERE blood_id=" + blood_id);
                out.println("<script src=\"js/alertme.js\"></script>");
                out.println("<script src=\"js/alert2.js\"></script>");
                out.println();
                out.println("<script>");
                out.println("$(document).ready(function() {");
                out.println("swal ( 'You Are' , 'Successfully Delete! ' ,' success '  ) ; ");
                out.println("}); ");
                out.println("</script>");
